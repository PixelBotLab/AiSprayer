...
def get_camera(camera_type="orbbec", width=1280, height=800, fps=15, timeout_ms=500):
    """
    根据类型获取相机驱动实例
    ...
    if camera_type == "orbbec":
        ...
        return OrbbecDriver(width=width, height=height, fps=fps, timeout_ms=timeout_ms)
