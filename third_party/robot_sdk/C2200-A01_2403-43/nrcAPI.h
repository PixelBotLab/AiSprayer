 ...
 * @brief 获取示教速度百分比
 * @return 返回示教速度百分比
 * @retval	返回值范围：0 < NRC_GetTeachRunSpeedPer()  <= 100 ,  NRC_GetTeachRunSpeedPer() != 0
 */
int NRC_GetTeachRunSpeedPer();

/**
 * @brief 获取示教模式下的定距移动
 * @param kind 查询定距移动的坐标系 1:关节 2:直角
 * @param stepMovement 定距移动的距离
 */
int NRC_GetTeachStepMovement(int kind, double& stepMovement);
...
/**
 * @brief 将机器人工具坐标值转换为关节坐标值(逆解参考点为零点)
 * @param posPCS 要进行转换的工具坐标值
 * @param posACS 转换的结果通过其返回
 * @return 返回是否转换成功
 * @retval  0 转换成功
 * @retval  -1 转换失败
 */
int NRC_PositionPCStoACS(const NRC_Position& posPCS, NRC_Position& posACS);

/**
 * @brief 将机器人关节坐标值转换为工具坐标值(逆解参考点为零点)
 * @param posACS 要进行转换的关节坐标值
 * @param posPCS 转换的结果通过其返回
 * @return 返回是否转换成功
 * @retval  0 转换成功
 * @retval  -1 转换失败
 */
int NRC_PositionACStoPCS(const NRC_Position& posACS, NRC_Position& posPCS);
 ...
 * @attention 需要和 NRC_RunqueueInsertWVOFF(int id) 配合使用
 */
int NRC_RunqueueInsertWVON(int id);
/**
 * @brief 作业文件插入WVON指令
 * @param id 摆焊工艺的工艺号，参数范围：1 <= id <= 99
 ...
 * @pre 调用该函数前，请先调用 NRC_CreateNoFlieRunqueue() 创建一个无文件运行队列
 * @param id 摆焊工艺的工艺号，参数范围：1 <= id <= 99
 * @attention 需要和 NRC_RunqueueInsertWVON(int id) 配合使用
 */
int NRC_RunqueueInsertWVOFF(int id);
/**
